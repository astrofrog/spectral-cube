...
{
    PyObject *m;
    MOD_DEF(m, "util", module_docstring, module_methods);
    if (m == NULL)
...
static PyObject *_combine_chunks(PyObject *self, PyObject *args)
{
    ...
    int ox, oy, oz, nx, ny, nz, nsx, nsy, nsz;
    npy_intp dims[1];
    double *output;
    NpyIter *iter;
    ...
    PyArray_Descr *dtype;
    int bx, by, bz, i, j, k, i_o, j_o, k_o, i_f, j_f, k_f;

    /* Parse the input tuple */
    if (!PyArg_ParseTuple(args, "Oiiiiii", &input_obj, &nx, &ny, &nz, &ox, &oy, &oz))
    {
                ...
                for (k = 0; k < nz; ++k)
                {
                    for (j = 0; j < ny; ++j)
                    {
                        for (i = 0; i < nx; ++i)
                        {
                            ...
                            index = i_f + j_f * (nx * ox) + k_f * (nx * ox * ny * oy);
